...
import { normalizeChargeGroupName } from "@/lib/migration/charge-group-sync";

export function ChargeObjectAssignDialog({
  open,
  onClose,
  group,
  allObjects,
  ...
  const [showOutsideGroup, setShowOutsideGroup] = useState(false);
  const [saving, setSaving] = useState(false);
  const groupName = normalizeChargeGroupName(group.name);

  const groupMemberIds = useMemo(
    () => new Set(group.objectIds ?? []),
    [group.objectIds],
  );
  ...
  const filtered = useMemo(
    () =>
      allObjects
        ...
        .filter((o) =>
          showOutsideGroup || groupMemberIds.has(o.id) || selected.has(o.id),
        )
    ...
    [allObjects, search, selected, showOutsideGroup, groupMemberIds],
  );
        ...
        <DialogHeader className="fiori-dialog-header fiori-dialog-header-rich shrink-0 space-y-0 text-left">
          <div className="fiori-dialog-header-row">
            <div className="fiori-dialog-header-main">
              ...
              <div className="fiori-charge-group-badge shrink-0" aria-hidden>
                {group.name}
              </div>
            ...
            <div className="fiori-dialog-header-actions">
              <label htmlFor="charge-assign-show-outside" className="fiori-dialog-header-filter">
                <Checkbox
                  id="charge-assign-show-outside"
                  checked={showOutsideGroup}
                  onCheckedChange={(checked) => setShowOutsideGroup(checked === true)}
                  className="fiori-dialog-header-filter-checkbox focus-visible:ring-0 focus-visible:ring-offset-0 [&_svg]:h-2.5 [&_svg]:w-2.5"
                  aria-label="Exibir objetos fora do grupo"
                />
                <span className="fiori-dialog-header-filter-label">
                  Exibir objetos fora do grupo
                </span>
              </label>
          ...
          <table className="fiori-report-table">
            ...
            <tbody>
              {filtered.map((obj) => {
                ...
                const currentGroup = obj.chargeGroup?.trim() || "—";
                const inOtherGroup =
                  !isSelected &&
                  normalizeChargeGroupName(obj.chargeGroup ?? "") !== "" &&
                  normalizeChargeGroupName(obj.chargeGroup ?? "") !== groupName;
                return (
                  <tr
                    key={obj.id}
                  ...
                  >
                    ...
                    <td className={cn("fiori-mono-muted", inOtherGroup && "text-amber-700")}>
                      {currentGroup}
                    </td>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="!border-0 p-0">
                    <div className="fiori-report-empty">
                      {showOutsideGroup || search
                        ? "Nenhum objeto encontrado"
                        : "Nenhum objeto neste grupo"}
                    </div>
                  </td>
                </tr>
              )}
